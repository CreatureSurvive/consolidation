...
- (id)init:(BOOL)onLockscreen {
    if (self = [super init]) {
        ...
        if (%c(NCMaterialView)) {
            ...
            selectedView.backgroundColor = [[CSCProvider sharedProvider] colorForKey:onLockscreen ? @"selectionColor" : @"ncSelectionColor"];
        }
...
- (void)updateView {
    ...
    for (NSString *appID in [notificationDict allKeys]) {
        PHAppView *appView = [[PHAppView alloc] initWithFrame:CGRectMake(0, 0, appViewSize(lockscreen).width, appViewSize(lockscreen).height) icon:iconForIdentifier(appID) identifier:appID numberStyle:[[CSCProvider sharedProvider] intForKey:lockscreen ? @"numberStyle" : @"ncNumberStyle"]];
    ...
    selectedView.backgroundColor = [[CSCProvider sharedProvider] colorForKey:lockscreen ? @"selectionColor" : @"ncSelectionColor"];
